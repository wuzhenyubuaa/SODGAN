...
class Generator(nn.Module):
    ...
    def forward(self, cond_vector, truncation):
        ...
        '''
        layers[0]: (batchsisze, 2048, 4, 4)
        layers[1]: (batchsisze, 2048, 8, 8)
        layers[2]: (batchsisze, 2048, 8, 8)
        layers[3]: (batchsisze, 1024, 16, 16)
        layers[4]: (batchsisze, 1024, 16, 16)
        layers[5]: (batchsisze, 1024, 32, 32)
        layers[6]: (batchsisze, 1024, 32, 32)
        layers[7]: (batchsisze, 512, 64, 64)
        layers[8]: (batchsisze, 512, 64, 64)
        layers[9]: (batchsisze, 512, 64, 64)  only input z, without cond_vector and truncation
        layers[10]: (batchsisze, 256, 128, 128)
        layers[11]: (batchsisze, 256, 128, 128)
        layers[12]: (batchsisze, 128, 256, 256)
        '''
...
class BigGAN(nn.Module):
    ...
    def forward(self, z, class_label, truncation):
        ...
        z, features = self.generator(cond_vector, truncation)
        return z, features
...
if __name__ == "__main__":
    ...
    from disciminator_wu import Discriminator
    import cv2

    batch_size =1

    cache_path = 'H:/mywork/pre-trained-model/biggan/256/pytorch_model.bin'
    saved_path =  'H:/mywork/work6/datasetGAN_release-master/data/biggan_sod/training_data/dear/'
    config = BigGANConfig(output_dim=256) # define the resolution 256
    model = BigGAN(config).cuda()

    model.load_state_dict(torch.load(cache_path))

    model.eval()

    # latent = np.load(saved_path+'latent.npy')
    # cond_vec = np.load(saved_path + 'cond_vec.npy')
    #
    # latent = torch.tensor(latent, dtype=torch.float).cuda()
    # cond_vec = torch.tensor(cond_vec,dtype=torch.float).cuda()
    #
    # for i in range(len(latent)):
    #
    #     with torch.no_grad():
    #         outputs, _ = model(latent[i], cond_vec[i], 0.4)
    #     # print(outputs.shape)
    #     print(' {} step'.format(i))
    #
    #     save_as_images(outputs, saved_path=saved_path+'img/', index=i)
    ...
    for epoch in range(10):
        ...
        for i in range(351, 353):

            noise = truncated_noise_sample(batch_size=batch_size, truncation=truncation)
            # label = one_hot_from_names('Eskimo dog', batch_size=batch_size)
            ...
            with torch.no_grad():
                outputs, _ = model(noise, label, truncation)
            # print(outputs.shape)
            print('{} epoch, {} step'.format(epoch, i))

            count +=1
            save_as_images(outputs, saved_path= saved_path, index=count)

    np.save( saved_path +'latent.npy', noise_list)
    np.save( saved_path + 'cond_vec.npy', label_list)

    #**********************************    discriminator    *************************************
    # outputs = nn.functional.interpolate(outputs,size=(128,128))
    # index = torch.argmax(label)
    #
    # D = Discriminator(D_ch=96)
    # D.load_state_dict(torch.load('D:/138k/D.pth'))
    # out = D(outputs, torch.tensor(torch.zeros(batch_size) + index, dtype=torch.long))
    # print(out)
